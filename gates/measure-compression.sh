...
# `build-and-run-*.sh` glob so run-all-gates.sh ignores it). The compression
# sibling of gates/measure-real-json.sh: it quantifies
# the unsupported-IL / unmapped-intrinsic surface the *real*
# System.IO.Compression DeflateStream/GZipStream path drags in, ahead of any
# zlib-vendoring / native-shim / transpiler production work.
#
# It transpiles samples/dotnet/CompressionProbe (a throwaway DeflateStream +
# GZipStream round-trip) in --measure mode (enumerates every reachable gap
# instead of stopping at the first), emitting NO C++.
#
...
OUT="${1:-artifacts/measure-compression}"

echo "== 1/4 Building CLI + CompressionProbe =="
build_proj src/Dn2Cpp.Cli/Dn2Cpp.Cli.csproj
build_proj samples/dotnet/CompressionProbe/CompressionProbe.csproj
CLI="src/Dn2Cpp.Cli/bin/$CONFIG/$TFM/dn2cpp.dll"
[ -f "$CLI" ] || { echo "error: not built: $CLI" >&2; exit 1; }

echo "== 2/4 Locating the real net10.0 CoreLib + System.IO.Compression =="
...
app="samples/dotnet/CompressionProbe/bin/$CONFIG/$TFM/CompressionProbe.dll"
[ -f "$app" ] || { echo "error: not built: $app" >&2; exit 1; }
mkdir -p "$OUT"

echo "== 3/4 Measure (real System.IO.Compression, --auto-ref) =="
tsv="$OUT/s0-gaps.tsv"
...
# to count the gaps the REAL BCL path drags in; with the shims injected, the
# P/Invoke rows they substitute stop being reachable and the numbers would
# silently describe a different subject.
dotnet exec "$CLI" "$app" -r "$corelib" -r "$REAL_COMPRESSION" \
