...
# user copies. The pin is the source; the rest are checked against it.
gdext_lib=$(first_line "$(sed -n 's/.*PROPERTIES OUTPUT_NAME \([A-Za-z0-9_.+-]*\).*/\1/p' \
                          runtime/CMakeLists.txt)")
if [ -z "$gdext_lib" ]; then
    bad "runtime/CMakeLists.txt has no OUTPUT_NAME pin on the GDEXTENSION target"
else
    ok "GDExtension OUTPUT_NAME pin = $gdext_lib"
    for f in samples/godot/*/dn2cpp.gdextension; do
        vals=$(awk '/^\[libraries\]/{s=1; next} /^\[/{s=0} s' "$f" \
               | sed -n 's/^[^=]*= *"\(.*\)"/\1/p')
        [ -n "$vals" ] || { bad "$f declares no [libraries] entries"; continue; }
        while IFS= read -r v; do
            case "$v" in
                "res://bin/lib$gdext_lib".*|"res://bin/$gdext_lib".*)
                    ;;
                *) bad "$f: '$v' does not name $gdext_lib" ;;
            esac
        done <<<"$vals"
        ok "$f names $gdext_lib throughout"
    done
    for want in "lib$gdext_lib.dylib" "$gdext_lib.dll" "$gdext_lib.gdextension"; do
        if grep -qF "$want" README.md; then
            ok "README.md spells $want"
        else
            bad "README.md does not spell '$want' — the GDExtension section is stale"
        fi
    done
fi
