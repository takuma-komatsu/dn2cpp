...
: "${UE_ROOT:?Set UE_ROOT to the existing Unreal Engine checkout}"
: "${UNREALSHARP_PLUGIN:?Set UNREALSHARP_PLUGIN to the patched plugin checkout}"
...
if [ -e "$project/Plugins/UnrealSharp" ] && [ "$(cd "$project/Plugins/UnrealSharp" && pwd -P)" != "$plugin" ]; then
    echo "error: smoke project already uses a different plugin" >&2
    exit 1
fi
[ -e "$project/Plugins/UnrealSharp" ] || ln -s "$plugin" "$project/Plugins/UnrealSharp"
