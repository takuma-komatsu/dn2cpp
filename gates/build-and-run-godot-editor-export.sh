...
# `--export-release` exits 0 even when an export plugin reported an error:
# EditorNode only fails the run when export_project() itself returns non-OK, and
# ExportPlugin._ExportBegin catches everything _ExportBeginImpl throws and turns
# it into an AddMessage(Error, …), which downgrades to "completed with warnings"
# (editor/editor_node.cpp:1407-1422). So a broken dn2cpp export produces a
# plausible-looking artifact and a zero exit code — these two assertions, not the
# exit code, are what makes this gate an oracle.
...
# below: the export plugin reported no error and the three dn2cpp stages ran. The
# exit code says nothing on its own (see 5/13).
...
# runs the platform exporter's own packaging afterwards (a plugin's error is a
# warning to the editor — see 5/13), and the good artifact plus its data dir sit
# in $OUT: sharing a directory would let one of these overwrite what 6/13 just
# asserted, or let a stale copy answer for a fresh one.
#
# godot_export_refused LOG WHAT — the half every refusal assert shares: the C#
# export plugin reported an error, and reported it before the publish. The
# caller then greps for the sentence identifying WHICH refusal fired, because
# all four of them look identical here.
...
# Exit code ignored on purpose, as in 5/13: an export plugin's error is a warning
# to the editor. A regression here does the whole publish and native build before
...
grep -q "ERROR: Export .NET Project" "$BAD_LOG" \
    || { echo "FAIL: the export plugin accepted a $FOREIGN_ARCH target on $ARCH" >&2
