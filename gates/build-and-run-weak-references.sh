...
# Image-resident Type objects retain weak identity without entering Boehm's
# heap-only disappearing-link table. Literal string headers live on the GC
# heap even though their character data is in the image; target replacement
...
ctx="weak_references|$_CG_CORELIB|runs:DN2CPP_GC_INCREMENTAL=0+1|DN2CPP_GC_STATS=1|assert:mode+diff+exit+generated-barriers+resize-ref+memmove-refs+pending-web-liveness+cwt-entry-scanned+cwt-section+immortal-weak-prefix+immortal-section"
...
expected_prefix=$(strip_cr_win "$expected")
before_prefix=$(strip_cr_win "$before")
assert_output "${expected_prefix%$'\n'immortal weak targets: True}" "$before_prefix" || assertions_failed=1
...
for run in "$stw" "$incremental"; do
    if [ "$(grep -cFx 'immortal weak targets: True' <<<"$(strip_cr_win "$run")")" -ne 1 ]; then
