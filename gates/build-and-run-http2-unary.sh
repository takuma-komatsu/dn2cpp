...
# own read=1, not an exception name.
grep -qF '[earlyreply] status=200 body=early-reply read=1' "$out/native.out" || {
...
# EXACTLY two lines, and each is selected BY NAME rather than by position: the
# sink's and the early-reply section's are the only length-less uploads in this
# run, a third would mean a sized body stopped going over whole at open(), and a
# missing one is a section that stopped feeding the send queue. Only the SINK's
# numbers are asserted — the early-reply upload is cut short by the reply, so its
# depth is not a stable fact, but its PRESENCE is: the server reads one body byte
# before answering, so that call's write path has provably run.
sinktag='dn2cpp: http2 send queue url=[^ ]*/sink parks='
earlytag='dn2cpp: http2 send queue url=[^ ]*/earlyreply parks='
sinkcount="$(grep -cE "$sinktag" "$out/native.err" || true)"
earlycount="$(grep -cE "$earlytag" "$out/native.err" || true)"
allcount="$(grep -cF 'dn2cpp: http2 send queue ' "$out/native.err" || true)"
[ "$sinkcount" = 1 ] || {
    echo "FAIL: expected exactly one send-queue stats line for /sink, found $sinkcount" >&2
    cat "$out/native.err" >&2; exit 1; }
[ "$earlycount" = 1 ] || {
    echo "FAIL: expected exactly one send-queue stats line for /earlyreply, found $earlycount — its incremental send path never ran" >&2
    cat "$out/native.err" >&2; exit 1; }
[ "$allcount" = 2 ] || {
    echo "FAIL: expected exactly two send-queue stats lines, found $allcount — a sized body stopped going over whole at open()" >&2
    cat "$out/native.err" >&2; exit 1; }
sinkstats="$(first_line "$(grep -E "$sinktag" "$out/native.err")")"
