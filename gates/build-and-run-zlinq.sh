...
gate_run_logs_init zlinq ZLinq
...
gate_run_diagnostics() {
    gate_run_diag "ZLinq native" "$native_code" "$native" "$native_stderr"
    gate_run_diag "ZLinq oracle" "$expected_code" "$expected" "$oracle_stderr"
}
