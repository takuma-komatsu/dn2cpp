...
# (C# 9), and InterfaceSealedMemberSubset (C# 8): sealed and private interface
# members run their own bodies through an interface, a delegate and a constrained
# call on a class or struct, and a constrained call on a struct binds a generic
# method's body as interface dispatch does. Appending is what keeps the
# output-order change at the end of the diff; the version each covers is named
# beside its call.
#
