#!/usr/bin/env bash
# The real MessagePack-CSharp 3.1.8 NuGet package and its source generator:
# indexed and map-keyed objects, ignored members and serialization constructors,
# collection formatters, interface unions, a local composite resolver with a custom
# formatter, direct reader/writer use, ReadOnlySequence input and Lz4BlockArray
# compression, exact-diffed against real .NET.
#
# The generated resolver and formatters land in the driver assembly. Their IL is part
# of the subject and the machinery-symbol assertion proves it reached the native tree.
source "$(dirname "$0")/_common.sh"

project=MessagePackSample
out="artifacts/messagepack"
measure="artifacts/messagepack-measure"
DLLS_EXPECTED=gates/expected/messagepack-dlls.sha256
MARKERS_EXPECTED=gates/expected/messagepack-markers.txt

echo "== 1/7 Locating the real net10 CoreLib =="
corelib=$(resolve_net10_corelib)
echo "corelib: $corelib"

echo "== 2/7 Building the driver against the real NuGet MessagePack-CSharp =="
nuget_packages="$(nuget_global_packages_root)"
if { [ ! -d "$nuget_packages/messagepack/3.1.8" ] \
    ...
    gate_skip "MessagePack-CSharp 3.1.8 and its pinned dependencies are not all in the NuGet cache and nuget.org is unreachable"
fi
...
if gate_cache_check "$out" \
        "messagepack|cli:$(_gate_cli_hash)|corelib=$corelib" \
...
if [ "$gaps" -ne 0 ]; then
    echo "FAIL: MessagePack-CSharp produced $gaps gap(s):" >&2
    LC_ALL=C cut -f2,3,5 "$measure/s0-gaps.tsv" | LC_ALL=C sed 's/^/        /' >&2
    exit 1
fi
echo "0 gaps, 0 cuts"

echo "== 5/7 The package and generated machinery MUST be in the tree =="
...
gate_run_logs_init messagepack MessagePack
...
gate_run_diagnostics() {
    gate_run_diag "MessagePack native" "$native_code" "$native" "$log_dir/native.log"
    gate_run_diag "MessagePack oracle" "$expected_code" "$expected" "$log_dir/oracle.log"
}
...
gate_cache_commit
echo "OK — the real MessagePack-CSharp 3.1.8 ran byte-identically to real .NET."
