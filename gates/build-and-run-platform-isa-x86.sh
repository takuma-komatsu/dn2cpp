...
# rest. AVX10.2 is false by default, and its positive opt-in is diffed against
# DOTNET_EnableAVX10v2 when the compiler can build those helpers; otherwise the
# opt-in still leaves the tokens false and calls their throwing stubs. Rows and
# Lowered flags come from the generated transpiler table, so a family promoted
# to Lowered joins the supported runs with no edit here.
...
# under the default policy and P the whole surface under the mask, with X86Base
# as the kept witness and Bmi1 as the removed one; _platform_isa.sh asserts each
# witness only while its family is in the Lowered set, and both are.
