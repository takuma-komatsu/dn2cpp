...
requires PIE to finish before writing its result. The scripting gate and PIE
...
For the official macOS UE build, build the Game target and finalize its app in
separate UBT calls. Set `configuration` to `Shipping` for that package:

```sh
project="$PWD/artifacts/unrealsharp/Baseline"
ubt="$UE_ROOT/Engine/Binaries/DotNET/UnrealBuildTool/UnrealBuildTool.dll"
configuration=Development
UE_BUILD_FROM_XCODE=1 dotnet "$ubt" Baseline Mac "$configuration" \
  "-Project=$project/Baseline.uproject" -UsePrecompiled -NoEngineChanges \
  -NoManifestChanges -NoHotReload -NoUBA -architecture=arm64
dotnet "$ubt" -Mode=ApplePostBuildSync \
  "-Input=$project/Intermediate/Build/Mac/arm64/Baseline/$configuration/PostBuildSync_Baseline.json" \
  "-remoteini=$project"
```

The installed build's immediate Xcode finalization can fail the `Touch UBT
generated tiles` pre-action; the separate post-build sync succeeds.

The cooked CLR Development baseline and native Development/Shipping packages
use the same gameplay artifact. Separate gates cover native relocation, signed
missing/incompatible library failures, and ILDiet-enabled/disabled artifact
parity. The packaged native runs retain sandbox entitlements and require CLR
absence. The package gates preserve signed entitlements.
