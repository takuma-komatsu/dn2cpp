...
リリースページの **Assets** 一覧から、必要な配布ファイルと `SHA256SUMS.txt` を同じフォルダへダウンロードします。macOS では展開する前に、ダウンロードしたファイルごとに次を実行してください。

```
shasum -a 256 <ダウンロードしたファイル名>
```

Windows には `shasum` がないため、PowerShell でダウンロードしたファイルごとに次を実行します。

```
Get-FileHash <ダウンロードしたファイル名> -Algorithm SHA256
```

出力されたハッシュを `SHA256SUMS.txt` の該当行と大文字小文字を無視して照合してください。


## インストール

エディタと、エクスポート先に必要なテンプレートはリリースページの **Assets** 一覧からダウンロードしてください。
...
表中の ● は必要、— は不要を表します。**.NET 10 SDK** がどのエクスポート先でも必要なのは、エクスポートがまずプロジェクトの IL をビルドするためです。dn2cpp が .NET ランタイムを取り除くのは*ゲーム*からであって、*エクスポートを実行するマシン*からではありません。テンプレートの入手方法は[エクスポートテンプレート](#エクスポートテンプレート)を、NDK と JDK の指定方法は[Android へのエクスポート](#android-へのエクスポート)を参照してください。

**macOS（Apple Silicon）ホスト**

| エクスポート先 | .NET 10 SDK | Xcode Command Line Tools | Xcode（full） | Python 3.10 以降 | Android NDK・JDK | エクスポートテンプレート |
|---|:-:|:-:|:-:|:-:|:-:|---|
| macOS | ● | ● | — | — | — | 本リリース同梱 |
| iOS | ● | ● | ● | — | — | Godot 公式 |
| Web | ● | — | — | ●※ | — | 本リリース同梱 |
...
**Web と Android へのエクスポートは Visual Studio 自体を必要としません。** どちらも MSVC ではなく、Emscripten と Android NDK でビルドするからです。ビルドを駆動する cmake と ninja もエディタが同梱しているため、Android へのエクスポートに別途用意するものは .NET 10 SDK と NDK・JDK だけです。Web も同様に、必要なのは .NET 10 SDK だけです。
...
下に挙げていないターゲットは、**標準の Godot .NET エクスポートテンプレート**をそのまま使います。使うのはリリース名 `<godot version>-dn2cpp.<X>.<Y>` の `<godot version>` に対応する upstream のバージョンで、通常どおりインストールしてください。

- **Windows** — Windows x86_64 のリリース用・デバッグ用エクスポートテンプレートが必要です。dn2cpp 出力の隣に配置されたネイティブ依存 DLL を探索する処理が入っています。
- **Web** — upstream は C# に対応した Web テンプレートを配布しておらず、また配布できないため、Web のリリース用・デバッグ用エクスポートテンプレートが必要です。
...
リリース Assets の `godot-<リリース名>-windows-x86_64-templates.zip` をダウンロードして展開します。Windows プリセットの **カスタムテンプレート → リリース**には `godot_windows_release_x86_64.exe`、**デバッグ**には `godot_windows_debug_x86_64.exe` を指定してください。2 つはビルド構成が異なるため、同じ exe を両方の欄に指定してはいけません。Visual Studio の C++ ワークロードも必要です（[動作要件](#動作要件)を参照）。エディタの起動方法は問いません。
...
- Assets の `godot-<リリース名>-web-templates.zip` を展開します。**カスタムテンプレート（リリース）**には内側の `godot_web_release.zip`、**カスタムテンプレート（デバッグ）**には `godot_web_debug.zip` を、それぞれアーカイブのまま指定します。外側の bundle を直接指定したり、同じ内側の zip を両方の欄に指定したりしてはいけません。
...
ゲームをコンパイルするのは、エディタが同梱する **Emscripten SDK** です。これは Web エクスポートテンプレート自体をビルドしたものと同じ SDK です。同じ SDK を使うのは単に手間を省くためではなく、ゲームはそのテンプレートの side module であり、両者は例外方式と動的リンクの ABI を一致させる必要があるためです。SDK も、それが起動する Node.js も同梱されているため、別途インストールするものはありません。

ブラウザのスタックトレースで managed 関数名を調べるときは、Web プリセットの **`dotnet/dn2cpp/keep_symbols`** を有効にしてください。Web 専用で既定は OFF です。有効にすると wasm の name section を残すため成果物は大きくなりますが、ブラウザが関数 index だけでなく `m_...` 形式の関数名を表示できます。この設定は Godot の「デバッグ付きエクスポート」（`--export-debug`）とは独立しており、リリースエクスポートでも使用できます。
...
- **時計は動きますが、分解能はブラウザのものです。** `Stopwatch` と `Environment.TickCount64` は他のターゲットと同じく単調に進みます。Web では `Stopwatch.Frequency` は 1,000,000,000 です — 単位は ticks/sec なので、1 tick = 1 ns という意味になります（デスクトップも同じとは限りません。Windows では QueryPerformanceCounter の周波数が返ります）。ただし**この値は分解能の保証ではありません**。実際に観測できる粒度はブラウザが時刻 API に課す丸めで決まり、報告値よりずっと粗くなります。フレーム時間そのものを測るなら `Godot.Time` 系のほうが素直です。
- **BCL のどこかが未対応だった場合、その症状はブラウザコンソールの `TypeError: resolved is not a function` です。** ゲームは side module としてロードされ、そこでは未解決のシンボルがリンクエラーではなく import になるため、書き出しもロードも成功し、**その関数を最初に呼んだ瞬間に**シンボル名を伴わないこのエラーだけが出ます。書き出しログには何も出ません。自分のコードの問題ではなくこちら側の穴なので、その行で何を呼んでいたかを添えて報告してください。
...
**エクスポートに失敗すると、エディタは失敗として報告します。** ヘッドレス実行（`--headless --export-release`）の終了コードも 0 以外になり、ログには `ERROR: Project export for preset "..." failed.` が出ます。ただし成果物の有無は判定に使えません — 失敗しても、それらしいファイルが残ることがあります。原因はログで確認してください。
...
- **Linux エディタは配布していません。** バックエンドは Linux をホストとしてもターゲットとしても扱うので、フォークを自分でビルドすればエクスポートできます。
