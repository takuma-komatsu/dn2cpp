...
リリース名の Godot バージョンに対応する公式 .NET エクスポートテンプレートを通常どおりインストールして使用します。カスタムテンプレートの指定は不要です。Visual Studio の C++ ワークロードも必要です（[動作要件](#動作要件)を参照）。エディタの起動方法は問いません。
...
ブラウザのスタックトレースで managed 関数名を調べるときは、Web プリセットの **`dotnet/dn2cpp/keep_symbols`** を有効にしてください。Web 専用で既定は OFF です。有効にすると wasm の name section を残すため成果物は大きくなりますが、ブラウザが関数 index だけでなく `型名_メソッド名_m...` 形式の関数名を表示できます。この設定は Godot の「デバッグ付きエクスポート」（`--export-debug`）とは独立しており、リリースエクスポートでも使用できます。
