...
| 41 | core | **Preserve the base-class chain of delegates declared outside CoreLib.** A delegate whose base is a `TypeRef` is marked as a delegate but left with `System.Object`, so `is Delegate` and casts fail after signal connection; in `Compilation` Pass 2 and the generic-specialization path, resolve and assign the `MulticastDelegate`/`Delegate` base as well as setting the delegate flag. Add a tail section to the existing frozen `ReflectTypes` bucket covering app-declared nested and specialized generic delegates, asserting the base chain, `is Delegate` / `is MulticastDelegate`, and both casts; prove the new block is identical to real .NET and the previous bucket output remains an unchanged prefix. |
