...
| 49 | runtime | **A multicast `Func<Task>` handed to `Task.Run`/`StartNew` runs its last handler alone.** The unwrap and nested arms of `dn2cpp_pool_worker` (`runtime/core/dn2cpp_tasks.cpp`) call the chain head's `d->method(d->target)` straight, so earlier handlers of a combined delegate never run — real .NET invokes every handler front-to-back and unwraps (or stores) the LAST handler's returned task. The result-kind thunks in the same file are the shape to mirror; the question a fix must answer first is what drives the earlier handlers' dropped inner tasks, whose timers and continuations land on the same worker-owned scheduler. The `TaskDelegateContractSubset` section of the AsyncCombinators bucket is where it gets its oracle-diffed coverage. |
