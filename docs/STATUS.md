...
| 21 | transpiler | **`__struct_size_pad` is still baked at 64-bit pointer width.** A `[StructLayout(Size = N)]` sequential value type whose natural field end depends on a pointer gets a trailing pad computed from the 8-byte model and emitted as a literal, so on wasm32 the struct is padded to the wrong total — the same defect the explicit-layout pad had. No such type occurs in either corpus measured (the `--dotnet-module` output and the whole gate suite), which is why it was left. Closing it needs more than swapping in `CppEmitter.ModelSize`: the emitter writes the pad only when it is `> 0`, and a type that needs no pad at 64 bits but does at 32 cannot be expressed by that branch — the shape has to change before the size can. Build the type first and let it go red, since nothing in the tree does. |
