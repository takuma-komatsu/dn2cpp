...
| 32 | runtime | **A dropped, never-read h2 response is never GC-torn-down on this Windows box.** `gates/build-and-run-http2-unary.sh`'s `[drop]` section (`samples/dotnet/Http2Unary/Program.cs`, `DropOverH2c`/`DropResponse`) abandons a `ResponseHeadersRead` response — no read, no `Dispose` — then loops 150x `GC.Collect()`+`GC.WaitForPendingFinalizers()`; it stays `server-started=True server-aborted=False`: `~Http2ResponseStream` (`internal/DnHttp/src/DnHttp/Http2StreamBackend.cs`), the only dropped-path caller of `_call.Abort`, never fires. MSVC-conditioned (this machine's `cl.exe` gate build; macOS/Linux stay green on the same code); `[cancel]` in the same bucket proves the abort plumbing, and no static table or `GCHandle` holds the message/response/stream past `DropResponse`. A `GC_DUMP_REGULARLY=1` run (drop loop cut to 3) shows the collector still MARKS it: finalization entries hold at 4 with 0 finalization-ready across every explicit collection — a live false root, not finalization machinery. Stack scrubbing is now tested and out: pool workers (`dn2cpp_pool_worker`, `runtime/core/dn2cpp_tasks.cpp`) never scrub and `Task.Run(() => call.WaitHeaders())` does land the call on a worker stack, but a noinline scrub injected at the worker's busy→idle transition flipped nothing at 16 KiB nor 256 KiB, and widening `dn2cpp_scrub_stack`'s own band (`runtime/core/dn2cpp_gc.cpp` — the `GC.Collect` caller and the finalizer thread) from 16 KiB to 256 KiB flipped nothing either; `GC_set_no_dls(1)` already confines static roots to the `dn2cpp_roots` section. Remaining suspects: a blocked thread's register context (win32 bdwgc pushes `GetThreadContext` registers as roots) and a stale word inside a live, conservatively scanned untyped-`GC_MALLOC` heap object (deterministic per build, matching the MSVC-only repro). Next step: rebuild the `dn2cpp_gc` target with `KEEP_BACK_PTRS`+`MAKE_BACK_GRAPH` (`runtime/CMakeLists.txt`, the non-Emscripten `target_compile_definitions`) and query `GC_get_back_ptr`/`GC_PRINT_BACK_HEIGHT` from a temporary hook in the drop loop to name the retaining root directly. |
