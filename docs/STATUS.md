...
| 31 | runtime | **Windows CI's `async-core` gate throws a false-positive deadlock, not a real one.** `TaskStateSubset.cs`'s `cold.Start(); await cold;`, and `Statuses().Result` beside it, are — by `Program.cs`'s `__GateEntry` order — the first place in the whole corpus that touches the real thread pool, and under `.github/workflows/windows-smoke.yml`'s real-MSVC `windows-latest` runner that throws an uncaught `InvalidOperationException`; macOS and Linux stay green on the same code. `dn2cpp_task_block`'s `dn2cpp_task_drain_settle` (`runtime/core/dn2cpp_tasks.cpp`) decides deadlock by asking `dn2cpp_task_settler_exists`, which reads `g_inflight_async_tasks` — incremented in `dn2cpp_pool_enqueue` before `dn2cpp_pool_ensure_started` spawns the first worker thread, decremented only after that worker has pushed its completion continuation via `dn2cpp_sched_enqueue`. A settle check landing inside that startup window sees no settler yet and misfires `dn2cpp_task_throw_deadlock`. No `_WIN32` branch touches this path, and the Windows PAL (`runtime/core/platform/windows/dn2cpp_pal_windows.cpp`) owns none of threads, tasks or GC-thread registration — so the leading theory is a race whose window a slow first `std::thread` spawn (shared-vCPU runner, Defender overhead) opens wide enough to hit, where a fast `pthread_create` never has. Next step is a timestamped trace across `dn2cpp_pool_enqueue`, `dn2cpp_pool_worker` and `dn2cpp_task_drain_settle`, looped on the Windows runner around the same `Start(); await` shape, to catch the actual interleaving before choosing a fix — the likely fix orders the inflight-counter decrement after the settler is genuinely observable, not before. |
| 32 | runtime | **A dropped, never-read h2 response is never GC-torn-down on this Windows box.** `gates/build-and-run-http2-unary.sh`'s `[drop]` section (`samples/dotnet/Http2Unary/Program.cs`, `DropOverH2c`/`DropResponse`) takes a `ResponseHeadersRead` response to `/drip` and abandons it — no read, no `Dispose` — then loops 150x `GC.Collect()` + `GC.WaitForPendingFinalizers()` waiting for the server's aborted counter. It stays `server-started=True server-aborted=False` the whole time: `Http2ResponseStream`'s finalizer (`internal/DnHttp/src/DnHttp/Http2StreamBackend.cs`, `~Http2ResponseStream`, the only path that calls `_call.Abort(...)` for a dropped-not-disposed stream) never fires. Confirmed by instrumenting that finalizer with a stderr print, rebuilding and rerunning: the string lands in the compiled `generated.cpp` but never prints across the whole 150-iteration/~15s loop, and a source grep of `Http2StreamBackend.cs`/`DnHttpBackend.cs` turns up no static table or `GCHandle` holding the message, response or stream past `DropResponse`'s return — so nothing legitimate keeps the object reachable. `[cancel]` right above it in the same file proves the abort plumbing itself works: that section drives `_call.Abort` explicitly (via a fired `CancellationToken`) and the server sees `server-aborted=True` every time. Reproduces identically on the pre-GC-fork snapshot and on every commit of the Unity-fork GC vendoring work, so it predates that switch and the write barriers — it is Windows- or MSVC-toolchain-conditioned (this machine's gate build lands in `artifacts/.cmake-runtime-cl`, i.e. `cl.exe`, not clang). Leading theory: a conservative Boehm scan finds a stale pointer to the dropped object in a stack/register slot `dn2cpp_scrub_stack`'s 16 KiB band (`runtime/core/dn2cpp_gc.cpp`) does not reach — `DropResponse`'s one-shot `SendAsync(...).GetAwaiter().GetResult()` call chain (thread-pool/`Task` plumbing) plausibly pushes deeper than the shallow `GetText`/`GC.Collect()` loop that runs afterward, so the band scrubbed before each collection never overlaps the address still holding the old pointer. Next step: widen or reposition the scrub band (or dump `GC_dump`/root output) across that specific call depth and confirm the pointer's address falls outside the scrubbed range. |
