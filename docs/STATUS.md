...
| 50 | intrinsics | **Lower the platform intrinsic families behind the ISA contract.** The contract itself is in place: the generated family table (`src/Dn2Cpp.Transpiler/CoreIntrinsics.PlatformIsa.g.cs`), the runtime's token header and CPU detection with the `DN2CPP_CPU_FEATURES` mask, and the x86 / Arm / Wasm / surface gates (`gates/build-and-run-platform-isa-surface.sh` and siblings). What remains is lowering the instruction surfaces so families can be promoted to Lowered, in this order of pay-off: the x86 scalar families (X86Base, Lzcnt, Popcnt, Bmi1, Bmi2, X86Serialize); the Arm scalar families (ArmBase, Crc32); AdvSimd with the Arm crypto families (Aes, Sha1, Sha256, Dp, Rdm); PackedSimd together with the wasm SIMD build axis (`DN2CPP_WASM_SIMD`) and the supported arm of the wasm gate; Sse through Sse42 with Pclmulqdq and Aes; Avx, Avx2, Fma and AvxVnni; the Avx512, Avx10, AvxVnniInt8/Int16 and Gfni families; Sve and Sve2 last. Promotion is never a hand edit: the surface gate decides it from the generator's map coverage (every public static method of the family has a helper, and a nested family's enclosing family is Lowered too), and the probe bucket's supported branch gains its exercise section with each promotion. Host-bound follow-ups: the x86 partial-mask knob pair in the x86 gate (`DN2CPP_CPU_FEATURES=-Avx` vs `DOTNET_EnableAVX=0`) is confirmed against `libclrjit` on an x86-64 host when the first X86 family is Lowered, and the first Lowered Arm family turns the arm gate's supported runs on for the macOS arm64 pre-merge host. |
