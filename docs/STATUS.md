...
| 50 | intrinsics | **Platform ISA contract: the host-bound and compiler-surface follow-ups.** Every family with feature bits is Lowered (the whole `X86.*`, `Arm.*` and `Wasm.*` surfaces; `Sve` and `Sve2` have no feature bits and answer false by design), so what remains is verification. Host-bound: a hosted windows-x64 rerun of the combined native gate must validate S across the full supported surface, including zero-divisor versus quotient-overflow `DivRem` exceptions, full-range SSE lane-index wrapping, and opaque VCMPSS/VCMPSD upper-lane preservation, then complete O (`DN2CPP_ENABLE_AVX10V2=1` vs `DOTNET_EnableAVX10v2=1` when the compiler can build it) and P (`DN2CPP_CPU_FEATURES=-Avx` vs `DOTNET_EnableAVX=0`, with X86Base kept and Bmi1 removed). Linux-x64 S/O/P remain unverified. No AVX-512 or AVX10 instruction body has executed on supporting hardware: `tools/platform-isa-rosetta.sh` runs the probe under Rosetta 2, which has no AVX, and is a developer procedure without the .NET oracle, not a gate. Compiler-surface: the AVX10.2 rows use current Clang intrinsic spellings; their compiler-capability guard makes the affected tokens false under GCC, MSVC and older Clang, so adding and verifying each compiler's spellings remains open. |
