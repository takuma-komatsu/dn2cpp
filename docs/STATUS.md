...
| 23 | transpiler | **A declared size below the natural one is rounded up, where .NET truncates to it.** `[StructLayout(LayoutKind.Sequential, Size = 12)] struct { IntPtr P; int I; }` measures 12 in real .NET through `Unsafe.SizeOf`, `Marshal.SizeOf` and the array stride alike, while dn2cpp emits a plain struct and C++ rounds it to the alignment, answering 16. The branch that should notice is the one returning no padding when the declared size does not exceed the natural one — it is silent, and the defect is independent of pointer width, so it is wrong at 64 bits too. Expressing the smaller total needs a shape C++ can hold at that size, which the declared-size union arm beside it cannot supply. `samples/dotnet/PInvokeNative/SequentialSizeSubset.cs` is where the section goes; the type is deliberately absent from it, since it reds for this reason rather than the one that section pins. |
| 24 | transpiler | **The sequential-layout `static_assert` states the 64-bit premise where it could pin the number.** `CppEmitter`'s explicit-layout and marshalled-layout asserts now write their size through `PointerWidth.Model` and are unguarded, so a wasm32 link checks those models against the C++ compiler's own layout. The sequential one still branches on the extent's `PtrFree` and emits `sizeof(void*) != 8 || …`, which is vacuously true on the one 32-bit target, so **every pointer-bearing value type in the corpus is unchecked there** — and that is the corpus, not the six marshal structs. The cost of leaving it is a missing check and nothing else: a plain sequential body is emitted as real typed fields, so the C++ compiler produces the layout and the model's number never fixes it. Closing it is the same edit the other two took — read the extent at `PointerWidth.Bytes32` as well and render the pair — with a blast radius of every value type rather than a handful, which is why it was kept separate. |
| 25 | transpiler | **A guarded `ModeledSize` is honoured at the asserts and ignored where it decides a layout.** `PointerWidth.Model` falls back to a bare 64-bit constant with `Guarded` set when the narrow reading is missing or the two readings are not a pointer count apart. Two sites take `.Text` regardless — the opaque value shell's `__opaque_pad` and the explicit layout's `__explicit_pad`, both of which ARE the struct's size — and the `marshalSize` stamp takes `.Int32Expr`, which drops the flag the same way; the explicit-size assert beside it then guards itself off, so a 64-bit size on wasm32 would be silent in exactly the case the fallback exists for. Reachability differs by site and has to be established per site rather than assumed: `ExplicitLayoutExtent`'s refusals are weakly monotone in the pointer width, which argues the explicit pad cannot reach it, while the marshalled stamp reaches it through a declared `Size` that lands between the two natural sizes. The fix is not a wider guard but a decision at each site about what a size the model could not write in `sizeof(void*)` should do — a refusal is available and a silent 64-bit number is not. |
