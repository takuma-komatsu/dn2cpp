...
| 38 | godot-editor | **Assert a Web export's import closure inside the exporter.** The gates assert it for the closures their own samples reach — `gates/build-and-run-godot-editor-export-web.sh`, its hermetic twin, `gates/build-and-run-cri-web.sh` and `gates/build-and-run-godot-dotnet-wasm.sh` — but a user's game reaching a PAL symbol the wasm build does not define is invisible to every one of them, and dies in the player's browser as `TypeError: resolved is not a function` naming a function index, with nothing in the export log. Doing the same check inside `Dn2CppExporter` needs two things the repository cannot hand over as they are: a wasm import/export section reader in C#, because `gates/_wasm_symbols.js` is a gate tool and is not among the siblings the toolchain bundle ships; and a point in the export where both the drop-in and the staged `index.wasm` + `index.js` are readable, which is after the platform exporter has run rather than where the backend builds. Measure where in the export order both exist before designing the hook. |
