...
| 35 | godot-editor | **Run the fork engine build on Linux, then accept the desktop editor-export gate there.** Every Linux arm of that lane is written and unexercised: `gates/setup-godot-fork.sh` (`SCONS_PLATFORM=linuxbsd`), `godot_fork_desktop_template`'s `linux_template.<arch>`, the gate's `dn2cpp-app-linux` preset and `linuxbsd` layout, and the fork's own `Dn2CppExporter` LinuxBSD arm — none has compiled or run, because the fork edits engine C++ (`modules/mono/mono_gd/gd_mono.cpp`, `platform/web/`) so `provide_binary` cannot reuse the pristine clone's binaries and must scons an editor and a template_release. Acceptance is `gates/build-and-run-godot-editor-export.sh` green on a Linux host with sections 9 and 12 the only expected partials; sections 10, 11 and 13 DO have reachable state there and a partial for any of them is a regression, not a port gap. Two things inside it have never run on Linux: `dist/package-toolchain.sh` (row 16's subject) and the fork's `build_assemblies.py --bundle-dn2cpp`. Until it is green the README drop-in row's Linux cell stays 🚧, and the fork-side C# change rides unverified. |
