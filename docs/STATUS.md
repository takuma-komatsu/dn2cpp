...
| 50 | intrinsics | **Implement target-aware platform intrinsics.** Cover the platform-specific families under `System.Runtime.Intrinsics.X86`, `System.Runtime.Intrinsics.Arm`, and `System.Runtime.Intrinsics.Wasm` as one capability contract rather than independent `IsSupported` constants. A getter must derive from the emitted target architecture and the executing CPU's feature set, never from the machine running the transpiler. On x86, AVX-family support requires CPUID plus OSXSAVE and XGETBV validation. Set `IsSupported` true for an ISA family only after every corresponding instruction body the reachable BCL or application may call can lower safely; unsupported families remain false so the existing BCL software fallback is preserved. Make the runtime CPU feature mask injectable for deterministic tests, and add target-specific gates for x86, Arm, and Wasm that cover both supported and masked-off paths. |
