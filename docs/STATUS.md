...
| 48 | runtime | **A multicast continuation handed to `Task.ContinueWith` runs its last handler alone on every result-returning kind.** The void kinds walk the delegate's `prev` chain through `dn2cpp_paramthread_invoke`; none of the others do — `DN2CPP_CONTWITH_I4`/`I8`/`R4`/`R8`/`REF` (`runtime/core/dn2cpp_tasks.cpp`) call `d->method(d->target, ante)` straight, and the struct kind's synthesized twin `TaskStructContWithThunk` (`src/Dn2Cpp.Transpiler/MethodCompiler.EnumAsync.cs`) does the same, so real .NET's front-to-back walk keeping the last handler's value is missing across the whole family. The `Task.Run`/`StartNew`/cold-ctor side already walks and is the shape to mirror. Fix all six kinds in one change: ContinueWith is at least self-consistent today, and a per-kind fix would make it disagree with itself. The `TaskDelegateContractSubset` section of the AsyncCombinators bucket is where it gets its oracle-diffed coverage. |
