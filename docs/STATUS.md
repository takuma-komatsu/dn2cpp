...
| 50 | intrinsics | **Platform ISA contract: the host-bound and build-cost follow-ups.** Every family with feature bits is Lowered (the whole `X86.*`, `Arm.*` and `Wasm.*` surfaces; `Sve` and `Sve2` have no feature bits and answer false by design), so what remains is verification and cost. Host-bound: the x86 gate's supported runs — S, the whole Lowered x86 surface diffed against real .NET unmasked, and P, `DN2CPP_CPU_FEATURES=-Avx` vs `DOTNET_EnableAVX=0` witnessed on X86Base kept and Bmi1 removed — have not run on an x86-64 host, so the exercise output of every Lowered x86 family (the `ref=` cross-checks, the `DivideByZeroException` the faulting `DivRem` cases print, the `ArgumentOutOfRangeException` the range-checked immediates print) is unverified against real .NET until the linux-x64 or windows-x64 lane runs `gates/build-and-run-platform-isa-x86.sh`. The AVX, AVX-512 and AVX10 helpers have executed nowhere: compiled for x86-64 with zero diagnostics, never run, because `tools/platform-isa-rosetta.sh` runs the probe under Rosetta 2, which has no AVX, and is a developer procedure without the .NET oracle, not a gate; the AVX10.2 rows use clang 21's intrinsic spellings and have not been compiled with GCC or MSVC. Build-cost: `dn2cpp_vectors.h` includes every generated family header into every translation unit, which the precompiled header absorbs but a `DN2CPP_PCH=OFF` build — and every Emscripten build, which never uses the PCH — pays in full now that the x86 headers are large; the emitter should include only the family headers a unit calls. |
