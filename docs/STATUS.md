...
| 26 | transpiler | **A `[MarshalAs]` that contradicts its field's pointer width is refused at 32 bits and silent at 64.** `Compilation.MarshalLayout`'s named-unmanaged arm returns `Refused` when the declared width and the field's natural width disagree, and both are read per pointer width — so `[MarshalAs(UnmanagedType.U8)] IntPtr P` is Known(8) at 64 bits and Refused at 32. The type therefore carries no `marshalSize` stamp, and the runtime answers `Marshal.SizeOf` from `instanceSize` instead: 4 on wasm32. No diagnostic names the contradiction at either width, and no gate covers the shape. Two things to establish before choosing the fix, neither trustable from here: what real .NET-32 answers for that field (an exception, or 8), and whether the refusal should be the transpile-time one the unrepresentable layouts get — the declaration is contradictory at every width, so refusing only where the walk happens to notice is the accident, not the design. |
