...
| 52 | unrealsharp | **Resolve the pinned UE macOS startup LLM race without requiring memory tracking.** Captured pre-runtime failures show AppKit/LaunchServices allocation exiting an LLM scope while startup clears tracker state, asserting in `FLLMTracker::PopTag` before the native game library loads. Package fixtures keep tracking enabled with `-LLM`; fresh-path positive and negative runs verify this mitigation. Check an upstream engine fix and ordinary launch behavior without that flag. UE source remains unchanged, and permanent memory-tracking overhead is not the desired resolution. |
