...
| 50 | intrinsics | **Lower the platform intrinsic families behind the ISA contract.** The contract itself is in place: the generated family table (`src/Dn2Cpp.Transpiler/CoreIntrinsics.PlatformIsa.g.cs`), the runtime's token header and CPU detection with the `DN2CPP_CPU_FEATURES` mask, and the x86 / Arm / Wasm / surface gates (`gates/build-and-run-platform-isa-surface.sh` and siblings). Lowered so far: the whole Arm surface (ArmBase, Crc32, AdvSimd, Aes, Sha1, Sha256, Dp, Rdm with their nested types), PackedSimd behind the `DN2CPP_WASM_SIMD` build axis with the frozen SIMD run of the wasm gate, and on x86 X86Base, X86Serialize, Sse, Sse2, Sse3, Ssse3, Sse41, Sse42, Popcnt, Pclmulqdq, Aes, Avx, Avx2, Fma, AvxVnni, Lzcnt, Bmi1 and Bmi2 with their X64 nested types. What remains, in this order of pay-off: the Avx512, Avx10, AvxVnniInt8/Int16 and Gfni families together with the V256/V512 nested types of Pclmulqdq and Gfni, which imply them; Sve and Sve2 last. Whether the Godot Web export turns the wasm SIMD axis on is a separate decision, not taken here. Promotion is never a hand edit: the generator derives Lowered from map coverage and the feature-bit implications (every public static method of the family has a helper, and every family it implies — enclosing type and .NET instruction-set parents alike — is Lowered too), the surface gate re-derives both, and the generated `Exercises.g.cs` exercises each promoted family in the probe. Host-bound follow-up: the x86 gate's supported runs (S, and P with the partial-mask pair `DN2CPP_CPU_FEATURES=-Avx` vs `DOTNET_EnableAVX=0`, witnessed on X86Base kept and Bmi1 removed) have not yet run on an x86-64 host, so the exercise output of every Lowered x86 family — the `ref=` cross-checks, the `DivideByZeroException` the faulting `DivRem` cases print and the `ArgumentOutOfRangeException` the range-checked immediates print — is unverified against real .NET until the linux-x64 or windows-x64 lane runs the gate; `tools/platform-isa-rosetta.sh` runs the same probe as an x86-64 binary under Rosetta 2, a developer procedure without the .NET oracle, not a gate, and Rosetta has no AVX, so the Avx, Avx2, Fma and AvxVnni helpers have executed nowhere yet — only compiled. Build-cost follow-up: `dn2cpp_vectors.h` includes every generated family header into every translation unit, which the precompiled header absorbs but a `DN2CPP_PCH=OFF` build — and every Emscripten build, which never uses the PCH — pays in full now that the Arm and x86 headers are large; the emitter should include only the family headers a unit calls. |
