...
| Delegate callbacks | a delegate parameter | a native function pointer | Parked in the per-delegate-type, GC-rooted **thunk pool** and passed as the stable slot-thunk address (`CppEmitter.EmitMarshalFnPtrThunks`). That is the only delegate→fnptr path: the native may STORE the pointer and later invoke it on a foreign executor thread. Every thunk registers that thread with the collector before touching managed state and unregisters short-lived threads at exit; an escaping managed exception is reported and terminates at the C boundary. The pool admits 8 distinct delegate instances per type for the process lifetime; slots cannot be reclaimed because native code may retain any published pointer indefinitely, and exhaustion traps loudly. | `PInvokeCallbackSubset`; the stored/deferred and foreign-thread lifetime in `PInvokeStoredCallbackSubset` |
