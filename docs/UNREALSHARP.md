...
UE source is not modified. `UE_ROOT` identifies the separately installed engine.
The ABI compatibility baseline is UE `5.8.2` and upstream UnrealSharp commit
...
[`d5ec8f27dd4c4597f8a6d51276da40e0732a9db7`](https://github.com/takuma-komatsu/UnrealSharp-dn2cpp/commit/d5ec8f27dd4c4597f8a6d51276da40e0732a9db7)
...
For a source checkout, explicitly register the plugin automation scripts:
...
| Real-engine check | Verified result |
|---|---|
| CLR Editor scripting | Gameplay fixture and saved Blueprint/map creation pass. |
| CLR PIE | Actual play-session entry, automatic managed BeginPlay, Blueprint override/call and PIE exit pass. |
| CLR cooked Development | Self-contained baseline passes the deterministic gameplay artifact. |
| Native cooked Development and Shipping | Official packaging pipeline and signed sandboxed apps pass the same gameplay results, with CLR absent. |
| Native shutdown | Reverse module shutdown, pending-worker completion with live handles, and harmless foreign callback arrival after shutdown pass. |
| Native ILDiet disabled | An independently built image matches the enabled image's gameplay, lifecycle and shutdown artifacts. |
| Native Development and Shipping relocation | Moved applications pass the gameplay and shutdown checks. |
| Signed native startup failures | Missing-library and incompatible-ABI fixtures fail with explicit diagnostics. |
