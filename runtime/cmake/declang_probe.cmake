...
if(NOT CMAKE_HOST_SYSTEM_NAME STREQUAL "Darwin" AND NOT (CMAKE_HOST_SYSTEM_NAME MATCHES "^(Linux|Windows)$" AND ANDROID_NDK))
    message(FATAL_ERROR "DeClang requires native macOS, or an Android target on macOS, Linux, or Windows")
endif()
# These paths are also interpolated into the probe's generated CMake project.
foreach(variable COMPILER WORK_DIR NINJA_EXE ANDROID_NDK)
...
dn2cpp_declang_home("${WORK_DIR}/disabled" disabled_home)
file(WRITE "${disabled_home}/.DeClang/config.json" "{\"enable_obfuscation\":0}\n")
set(ENV{DECLANG_HOME} "${disabled_home}")
...
if(ANDROID_NDK)
    ...
    set(ndk_root "${ANDROID_NDK}/toolchains/llvm/prebuilt/${ndk_host}")
    if(NOT EXISTS "${ndk_root}/bin/clang${host_suffix}")
        message(FATAL_ERROR "Android DeClang requires an NDK toolchain for ${ndk_host}")
    endif()
else()
...
file(SHA256 "${CMAKE_CURRENT_LIST_DIR}/declang_timestamp.ps1" timestamp_hash)
file(SHA256 "${CMAKE_CURRENT_LIST_DIR}/declang_home.cmake" home_hash)
set(identity "${compiler_hash}\n${probe_hash}\n${launcher_hash}\n${integration_hash}\n${timestamp_hash}\n${home_hash}\n${resources}\n${sdk}\n${DEPLOYMENT_TARGET}\n")
file(GLOB_RECURSE resource_files FOLLOW_SYMLINKS LIST_DIRECTORIES FALSE "${resources}/*")
...
if(ANDROID_NDK)
    ...
    foreach(tool clang clang++ ld.lld llvm-ar llvm-ranlib llvm-readelf llvm-strip)
        list(APPEND ndk_files "${ndk_root}/bin/${tool}${host_suffix}")
    endforeach()
    list(SORT ndk_files)
...
if(ANDROID_NDK)
    execute_process(COMMAND "${ndk_root}/bin/llvm-readelf${host_suffix}" -h "${probe_dir}/build/probe" OUTPUT_VARIABLE elf_header RESULT_VARIABLE status)
