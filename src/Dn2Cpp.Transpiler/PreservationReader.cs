...
internal sealed class PreservationReader
{
    ...
    internal sealed class PreservePolicy
    {
    ...
    private PreservePolicy Policy(PreservationModule module, TypeDefinitionHandle type)
    {
    ...
    private void DiscoverPreservationPolicies(bool includeAttributes)
    {
        ...
        foreach (string path in FindLinkFiles(_projectRoots).Concat(_linkXmlFiles).Distinct(StringComparer.Ordinal))
    ...
    private static void AddBackingField(PreservationModule module, TypeDefinition td,
    ...
    private bool HasPreserveAttribute(PreservationModule module, CustomAttributeHandleCollection attributes)
    {
    ...
    private bool IsRawPreserveAttributeType(PreservationModule module, TypeDefinitionHandle handle)
    {
    ...
    private bool RawBaseIsPreserve(PreservationModule module, TypeDefinitionHandle handle,
    ...
    private (PreservationModule Module, TypeDefinitionHandle Handle)? RawAttributeTypeDefinition(
        PreservationModule module, CustomAttribute attribute)
    {
    ...
    private (PreservationModule Module, TypeDefinitionHandle Handle)? RawTypeReferenceDefinition(
        PreservationModule module, TypeReferenceHandle handle)
    {
        ...
        PreservationModule? target = tr.ResolutionScope.Kind switch
        {
    ...
    /// <paramref name="parent"/>::<paramref name="member"/>, or a method group over it,
    /// constructs a type chosen at run time. The transpiler then reaches every application
    /// instance constructor, and ILDiet keeps the ones a type token or a custom-attribute
    /// Type argument can select; both arm on this predicate.</summary>
    ...
    internal static IReadOnlyList<string> FindLinkFiles(IReadOnlyList<string> projectRoots)
    {
        ...
        foreach (string rootValue in projectRoots)
        {
    ...
    private bool IsLinkAssemblyReferenced(PreservationModule target)
    {
        if (target == AppModule)
            return true;
        foreach (var module in Modules)
    ...
    private void ApplyLinkType(string path, PreservationModule module, LinkNode type)
    {
        if (type.Name != "type")
            throw LinkError(path, $"unknown <{type.Name}> under <assembly>");
        RequireOnly(type, path, new[] { "fullname", "name", "preserve", "required", "feature" });
    ...
    private void ApplyLinkMember(string path, PreservationModule module, TypeDefinitionHandle tdh,
    ...
    private bool MethodSignatureMatches(PreservationModule module, MethodDefinitionHandle handle, string selector)
    {
    ...
    private bool FieldSignatureMatches(PreservationModule module, FieldDefinitionHandle handle, string selector)
    {
    ...
    private bool PropertySignatureMatches(PreservationModule module, TypeDefinitionHandle declaring,
    ...
    private bool EventSignatureMatches(PreservationModule module, TypeDefinitionHandle declaring,
    ...
    {
        if (_warnings) Console.Error.WriteLine($"warning: {path}: link.xml: {message}");
    }

    private readonly IReadOnlyList<string> _linkXmlFiles;
    private readonly IReadOnlyList<PreservationModule> Modules;
    private PreservationModule AppModule => Modules[0];

    internal PreservationReader(IReadOnlyList<PreservationModule> modules,
        IReadOnlyList<string> projectRoots, IReadOnlyList<string> linkXmlFiles,
        IReadOnlyList<string> features)
    {
        Modules = modules;
        _projectRoots = projectRoots;
        _linkXmlFiles = linkXmlFiles;
        _linkFeatures = new HashSet<string>(features, StringComparer.Ordinal);
        foreach (string feature in features)
            if (feature is not ("com" or "sre" or "remoting"))
                throw new NotSupportedException("link feature must be com, sre, or remoting: " + feature);
    }

    internal Dictionary<(int Module, TypeDefinitionHandle Type), PreservePolicy> Read(bool includeAttributes = true, bool warnings = true)
    {
        _warnings = warnings;
        DiscoverPreservationPolicies(includeAttributes);
        return _preservePolicies;
    }

    private Dictionary<(string, string), List<(PreservationModule, TypeDefinitionHandle)>>? _typeIndex;
    private Dictionary<(string, string), List<(PreservationModule, TypeDefinitionHandle)>> TypeIndex()
    {
        if (_typeIndex is not null) return _typeIndex;
        var index = new Dictionary<(string, string), List<(PreservationModule, TypeDefinitionHandle)>>();
        foreach (var module in Modules)
            foreach (var handle in module.Reader.TypeDefinitions)
            {
                var type = module.Reader.GetTypeDefinition(handle);
                var key = (module.Reader.GetString(type.Namespace), module.Reader.GetString(type.Name));
                if (!index.TryGetValue(key, out var rows)) index.Add(key, rows = new());
                rows.Add((module, handle));
            }
        return _typeIndex = index;
    }

    private static bool IsMustEscape(Exception exception) => exception is OutOfMemoryException;
    ...
    private static string? AttributeTypeName(MetadataReader reader, CustomAttribute attribute)
    {
        ...
        {
            HandleKind.TypeDefinition => RawSignatureProvider.TypeDefinitionName(reader, (TypeDefinitionHandle)type),
            HandleKind.TypeReference => RawSignatureProvider.TypeReferenceName(reader, (TypeReferenceHandle)type),
            _ => null,
        };
    }
}

internal sealed class PreservationModule
{
    internal required int Index;
    internal required string AssemblyName;
    internal required MetadataReader Reader;
}
