...
internal static class CppTypes
{
    ...
    /// inline struct, or a width-naming descriptor equal to the field's natural native width
    /// (<c>[MarshalAs(U4)] uint</c>). Which field KINDS may carry a descriptor at all is not
    /// decided here — that is <see cref="Compilation.MarshalDescriptorKindAllows"/>, the one
    /// row the marshalled-layout model asks too, because a disagreement between the two is
    /// silent: one would size a struct .NET refuses to load. Everything
