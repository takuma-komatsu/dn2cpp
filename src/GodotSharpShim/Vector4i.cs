...
namespace Godot
{
    /// <summary>Hand-written members for the generated <c>Vector4i</c> value type.
    /// The fields (<c>X</c>/<c>Y</c>/<c>Z</c>/<c>W</c>), constructors, arithmetic
    /// and equality operators, and constants come from the binding generator; the
    /// engine dump carries no relational-operator bodies, so they are hand-written
    /// here. Layout stays <c>{X, Y, Z, W}</c>.</summary>
    public partial struct Vector4i
    {
        // Relational operators — Godot's are LEXICOGRAPHIC (compare X, then Y, then
        // Z, then W), not component-wise. Verbatim from Godot's C# Vector4I.cs.
