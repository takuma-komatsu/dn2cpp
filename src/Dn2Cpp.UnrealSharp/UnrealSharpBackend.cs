...
internal sealed class UnrealSharpBackend : IEmitBackend
{
    ...
    public void EmitEpilogue(CppEmitter emitter, StringBuilder sb, IReadOnlyList<MethodInfo> cctors)
    {
        ...
        sb.AppendLine("    if (host->ue_major != 5 || host->ue_minor != 8 || host->ue_patch != 3 || !host->unrealsharp_revision || std::strcmp(host->unrealsharp_revision, DN2CPP_UNREALSHARP_REVISION)) return dn2cpp_us_result(result, \"UnrealSharp or UE compatibility mismatch\");");
