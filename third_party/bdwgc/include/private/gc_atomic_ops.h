...
#   define AO_CLEAR(p) \
        ((void)_InterlockedExchange8((volatile char *)(p), (char)AO_TS_CLEAR))
#   define AO_test_and_set_acquire(p) \
        ((AO_TS_t)_InterlockedExchange8((volatile char *)(p), (char)AO_TS_SET))
#   define AO_HAVE_test_and_set_acquire

#   define AO_compiler_barrier() _ReadWriteBarrier()
#   define AO_nop_full() _mm_mfence()
#   define AO_HAVE_nop_full

#   define AO_fetch_and_add(p, v) \
        ((AO_t)_InterlockedExchangeAdd64((volatile __int64 *)(p), (__int64)(v)))
#   define AO_HAVE_fetch_and_add
#   define AO_fetch_and_add1(p) AO_fetch_and_add(p, 1)
#   define AO_HAVE_fetch_and_add1

#   define AO_or(p, v) \
        ((void)_InterlockedOr64((volatile __int64 *)(p), (__int64)(v)))
#   define AO_HAVE_or

#   define AO_load(p) (*(const volatile AO_t *)(p))
#   define AO_HAVE_load
#   define AO_load_acquire(p) (*(const volatile AO_t *)(p))
#   define AO_HAVE_load_acquire
#   define AO_load_acquire_read(p) AO_load_acquire(p)
#   define AO_HAVE_load_acquire_read

#   define AO_store(p, v) (*(volatile AO_t *)(p) = (v))
#   define AO_HAVE_store
#   define AO_store_release(p, v) (*(volatile AO_t *)(p) = (v))
#   define AO_HAVE_store_release
#   define AO_store_release_write(p, v) AO_store_release(p, v)
#   define AO_HAVE_store_release_write

#   define AO_char_load(p) (*(const volatile unsigned char *)(p))
#   define AO_HAVE_char_load
#   define AO_char_store(p, v) (*(volatile unsigned char *)(p) = (v))
...
# ifdef AO_REQUIRE_CAS
    ...
    {
      return (AO_t)_InterlockedCompareExchange64((volatile __int64 *)p,
                                        (__int64)nv, (__int64)ov) == ov;
    }

    AO_INLINE int
    AO_compare_and_swap_release(volatile AO_t *p, AO_t ov, AO_t nv)
    {
      /* The lock-prefixed cmpxchg _InterlockedCompareExchange64 lowers to is
         already a full fence, so no weaker-than-release variant exists to
         call instead. */
      return AO_compare_and_swap(p, ov, nv);
    }
...
# endif /* GNU/Clang __atomic builtins */
